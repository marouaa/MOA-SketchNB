 ...
 */
public class CountMinSketch2 implements IFrequency, Serializable {
    ...
    public CountMinSketch2(int depth, int width, int seed) {
        this.depth = depth;
        this.width = width;
        this.eps = Math.exp(1.0) / width;
        this.confidence = 1 - 1 / Math.pow(Math.exp(1.0), depth);
        initTablesWith(depth, width, seed);
    }

    public CountMinSketch2(double epsOfTotalCount, double confidence, int seed) {
        // 2/w = eps ; w = 2/eps
        // 1/2^depth <= 1-confidence ; depth >= -log2 (1-confidence)
        this.eps = epsOfTotalCount;
        this.confidence = confidence;
        this.width = (int) Math.ceil(Math.exp(1.0) / epsOfTotalCount);
        this.depth = (int) Math.ceil(Math.log(1.0 / (1.0-confidence)));
        initTablesWith(depth, width, seed);
    }

    CountMinSketch2(int depth, int width, long size, long[] hashA, long[][] table) {
        this.depth = depth;
        this.width = width;
        this.eps = Math.exp(1.0) / width;
        this.confidence = 1 - 1 / Math.pow(Math.exp(1.0), depth);
    ...
    public boolean equals(Object o) {
        ...
        final CountMinSketch2 that = (CountMinSketch2) o;
  ...
  public int getDepth() {
     ...
     */
    public static CountMinSketch2 merge(CountMinSketch2... estimators) throws CMSMergeException {
        CountMinSketch2 merged = null;
        if (estimators != null && estimators.length > 0) {
            ...
            for (CountMinSketch2 estimator : estimators) {
            ...
            merged = new CountMinSketch2(depth, width, size, hashA, table);
        }

        return merged;
    }

    public static byte[] serialize(CountMinSketch2 sketch) {
    ...
    public static CountMinSketch2 deserialize(byte[] data) {
        ByteArrayInputStream bis = new ByteArrayInputStream(data);
        DataInputStream s = new DataInputStream(bis);
        try {
            CountMinSketch2 sketch = new CountMinSketch2();
