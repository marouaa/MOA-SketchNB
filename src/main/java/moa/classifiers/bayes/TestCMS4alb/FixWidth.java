...
// THis class is about to fix the width of the sketch which will allow us to fix thhe width according to the accuracy 
	...
	//protected double[][] attributeObserversSum;
      //   protected AutoExpandVector <Double>  attributeObserversSum; 
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (start == true) {
			// this.observedClassDistribution = new double[inst.numClasses()]; 
                        this.observedClassDistribution = new DoubleVector();
			 //this.attributeObserversSum =  new AutoExpandVector<Double>();
	...
	public double[] doNaiveBayesPrediction(Instance inst) {
		...
		for (int classIndex = 0; classIndex < votes.length; classIndex++) {
			votes[classIndex] = this.observedClassDistribution.getValue(classIndex)
		...
		*/
                         votes[classIndex] *=((double) cmsketch.getEstimation(Integer.valueOf(""+(attIndex+1)+(int)inst.value(attIndex)+classIndex)))
                                                    /this.observedClassDistribution.getValue(classIndex);
				}
                }
