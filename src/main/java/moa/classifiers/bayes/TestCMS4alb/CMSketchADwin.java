...
public class CMSketchADwin { 
	...
	public CMSketchADwin(int width, int depth){
		this.depth = depth;
		this.width = width;
		this.count = 0;
                this.AdwinErr = new ADWIN[depth][width];
	...
	public double getEstimation(int query){
		...
		double ans = this.AdwinErr[0][hash31(this.hasha[0],this.hashb[0],query) % this.width].getTotal();
		for (int j=1;j<this.depth;j++){
			ans = Math.min(ans,this.AdwinErr[j][hash31(this.hasha[j],this.hashb[j],query) % this.width].getTotal());
		}
