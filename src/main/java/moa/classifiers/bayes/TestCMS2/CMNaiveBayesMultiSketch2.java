...
public class CMNaiveBayesMultiSketch2 extends AbstractClassifier implements MultiClassClassifier {
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (start == true) {
			 ...
			 this.cmsketch = new CountMinSketch2[this.numClasses];
			 for (int i=0; i<this.numClasses; i++){
				this.cmsketch[i] = new CountMinSketch2(this.epsilonOption.getValue(),
		...
		for (int i = 0; i < inst.numAttributes() - 1; i++) {
			...
			//this.attributeObservers[i][(int)inst.classValue()][(int)inst.value(i)] +=inst.weight();
			 String updateS = Integer.toString(i)+
                        ...
                        this.cmsketch[(int)inst.classValue()].add(updateS,1);
	...
	public double[] doNaiveBayesPrediction(Instance inst) {
		...
		for (int classIndex = 0; classIndex < votes.length; classIndex++) {
			...
			for (int attIndex = 0; attIndex < inst.numAttributes() - 1; attIndex++) {
				...
				votes[classIndex] *= ((double) cmsketch[classIndex].estimateCount(updateS))
