...
public class CompareProbabilities extends AbstractClassifier implements MultiClassClassifier {
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (start == true) {
			 ...
			 this.cmsketch = new CountMinSketch1(this.deltaOption.getValue(),this.epsilonOption.getValue());
	...
	public double[] doNaiveBayesPrediction(Instance inst) {
		...
		for (int classIndex = 0; classIndex < votes.length; classIndex++) {
				...
				votes[classIndex] *= ((double) this.cmsketch.getEstimatedCount(chaine))
          ...
          //  for (int attIndex = 0; attIndex < inst.numAttributes() - 1; attIndex++) {
            //             String chaine = Integer.toString(attIndex)+
              //                           Double.toString(inst.value(attIndex))+
                //                         Double.toString(indexVal(votes,maxTable(votes)));
                  //      Integer countlst = this.list.get(chaine);
