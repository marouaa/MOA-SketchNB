...
public class CMNaiveBayesONESKECTCH1 extends AbstractClassifier implements MultiClassClassifier {
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (start == true) {
			 ...
			 this.cmsketch = new CountMinSketch();
			
                         this.cmsketch = new CountMinSketch((int) Math.ceil(Math.exp(1)/this.epsilonOption.getValue()),(int) Math.ceil(Math.log(1.0/this.deltaOption.getValue() )));
		...
		for (int i = 0; i < inst.numAttributes() - 1; i++) {
			...
			this.cmsketch.setString(updateS); //? int weight
		...
		for (int classIndex = 0; classIndex < votes.length; classIndex++) {
			...
			for (int attIndex = 0; attIndex < inst.numAttributes() - 1; attIndex++) {
				...
				votes[classIndex] *= ((double) cmsketch.getEstimatedCount(chaine))
				 ...
				 int count = this.cmsketch.getEstimatedCount(chaine);	
                                 Integer countlst;
                        ...
                        System.out.println(this.error/(this.observedClassSum*(this.NumAtt-1)*votes.length));
